...
vec3 calDirLight(DirLight light, vec3 norm, vec3 viewDir)
{
	...
	//1.环境光
	vec3 ambient = vec3(texture(material.texture_diffuse, texCoord)) * light.ambient;

	//2.漫反射光
	float diff = max(dot(norm, lightDir), 0.0);
	vec3 diffuse = (diff * vec3(texture(material.texture_diffuse, texCoord))) * light.diffuse;

	//3.镜面反射光
	vec3 reflectDir = reflect(-lightDir, norm);
	float spec = pow(max(dot(reflectDir, viewDir), 0.0), material.shininess);
	vec3 specular = (spec * vec3(texture(material.texture_specular, texCoord))) * light.specular;
